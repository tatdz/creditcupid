"""
agent.py — Advisor (local)
- Receives ASI Ask{ message, account, chains? }
- Calls Analyst via analyst_protocol and waits (timeout)
- (Optional) Calls Risk via risk_protocol; if absent, uses fallback scoring
- Replies with Answer: reply (NL) + JSON {score, rationale, actions, preview}

ENV:
  ADVISOR_SEED=advisor_dev_seed
  ANALYST_ADDR=agent1q... (printed by analyst.py)
  RISK_ADDR= (optional; leave empty for fallback scoring)
  REQUEST_TIMEOUT=3.5
"""
...
# ----- Risk Protocol (optional for now) -----
...
REQUEST_TIMEOUT = float(os.getenv("REQUEST_TIMEOUT", "3.5"))

agent = Agent(name="darma_advisor", seed=ADVISOR_SEED,port=8002,endpoint=["http://127.0.0.1:8002/submit"])
...
async def on_analyst_response(ctx: Context, msg: DataResponse):
...
async def ask_risk(ctx: Context, summary: dict, by_chain: List[dict]) -> Optional[dict]:
    ...
    await ctx.send(RISK_ADDR, ScoreRequest(request_id=rid, summary=summary, by_chain=by_chain))
...
async def on_risk_response(ctx: Context, msg: ScoreResponse):
...
def fallback_scoring(summary: dict) -> dict:
    ...
    return {
        ...
        "actions": [{"id": "maintain", "label": "Maintain low utilization", "payload": {"type": "advice"}}],
    }

# ----- ASI Chat Handler -----
@ASI_CHAT.on_message(model=Ask, replies={Answer})
async def handle_chat(ctx: Context, msg: Ask):
    ...
    await ctx.send(msg, Answer(reply=reply, score=score, rationale=rationale, actions=actions, preview=preview))
