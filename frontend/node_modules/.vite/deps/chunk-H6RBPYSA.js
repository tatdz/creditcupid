...
P.cmp = function(y4) {
  ...
  if (!xc2[0] || !yc2[0])
    return !xc2[0] ? !yc2[0] ? 0 : -j : i3;
  if (i3 != j)
    return i3;
  isneg = i3 < 0;
  if (k2 != l3)
    return k2 > l3 ^ isneg ? 1 : -1;
...
P.minus = P.sub = function(y4) {
  ...
  if ((b = (j = yc2.length) - (i3 = xc2.length)) > 0)
    for (; b--; )
      xc2[i3++] = 0;
  for (b = i3; j > a; ) {
    if (xc2[--j] < yc2[j]) {
...
// node_modules/valtio/node_modules/proxy-compare/dist/index.modern.js
var e = Symbol();
var t = Symbol();
...
function Xt(t2, ...e2) {
  ...
  if (e2.length > 0 && !e2.includes(t2.length))
    throw new Error("Uint8Array expected of length " + e2 + ", got length=" + t2.length);
}
function Ce(t2) {
...
function Lt(t2 = 32) {
  if (_t && typeof _t.getRandomValues == "function")
    return _t.getRandomValues(new Uint8Array(t2));
  if (_t && typeof _t.randomBytes == "function")
    return _t.randomBytes(t2);
...
for (let t2 = 0, e2 = Jt, n2 = 1, r2 = 0; t2 < 24; t2++) {
  ...
  for (let i3 = 0; i3 < 7; i3++)
    e2 = (e2 << Jt ^ (e2 >> Hi) * Ki) % qi, e2 & Di && (o2 ^= Jt << (Jt << BigInt(i3)) - Jt);
  nr.push(o2);
}
...
function Zi(t2, e2 = 24) {
  ...
  for (let r2 = 24 - e2; r2 < 24; r2++) {
    for (let s2 = 0; s2 < 10; s2++)
      n2[s2] = t2[s2] ^ t2[s2 + 10] ^ t2[s2 + 20] ^ t2[s2 + 30] ^ t2[s2 + 40];
    ...
    for (let s2 = 0; s2 < 50; s2 += 10) {
      for (let c2 = 0; c2 < 10; c2++)
        n2[c2] = t2[s2 + c2];
      for (let c2 = 0; c2 < 10; c2++)
        t2[s2 + c2] ^= ~n2[(c2 + 2) % 10] & n2[(c2 + 4) % 10];
    }
...
var En = class _En extends ke {
  constructor(e2, n2, r2, o2 = false, i3 = 24) {
    if (super(), this.blockLen = e2, this.suffix = n2, this.outputLen = r2, this.enableXOF = o2, this.rounds = i3, this.pos = 0, this.posOut = 0, this.finished = false, this.destroyed = false, Wt(r2), 0 >= this.blockLen || this.blockLen >= 200)
...
function Br(t2, e2, n2 = true) {
  ...
  if (e2.length !== t2)
    throw new Error("invalid output length, expected " + t2 + ", got: " + e2.length);
  if (n2 && !vs(e2))
...
function Ir(t2, e2, n2, r2) {
  if (typeof t2.setBigUint64 == "function")
    return t2.setBigUint64(e2, n2, r2);
...
function Is(t2, e2, n2, r2, o2, i3, s2, c2) {
  ...
  for (let E2 = 0; E2 < a; s2++) {
    ...
    if (f6 && p === ge) {
      ...
      for (let v = 0, m2; v < Bs; v++)
        m2 = d2 + v, y4[m2] = h4[m2] ^ l3[v];
      E2 += ge;
      continue;
    }
    for (let d2 = 0, v; d2 < p; d2++)
      v = E2 + d2, i3[v] = o2[v] ^ u2[d2];
    E2 += p;
  }
}
function Ns(t2, e2) {
  ...
  return Fe(o2), Fe(s2), Ar(i3), Ar(n2), (c2, a, u2, l3, f6 = 0) => {
    ...
    if (E2 === 32)
      y4.push(p = Ze(c2)), d2 = As;
    else if (E2 === 16 && n2)
      p = new Uint8Array(32), p.set(c2), p.set(c2, 16), d2 = Os, y4.push(p);
    else
      throw new Error(`arx: invalid 32-byte key, got length=${E2}`);
    Ye(a) || y4.push(a = Ze(a));
    const v = mt(p);
    if (r2) {
      if (a.length !== 24)
...
var Us = class {
  ...
  finalize() {
    ...
    let o2 = e2[1] >>> 13;
    e2[1] &= 8191;
    for (let c2 = 2; c2 < 10; c2++)
      e2[c2] += o2, o2 = e2[c2] >>> 13, e2[c2] &= 8191;
    ...
    for (let c2 = 1; c2 < 10; c2++)
      r2[c2] = e2[c2] + o2, o2 = r2[c2] >>> 13, r2[c2] &= 8191;
    r2[9] -= 8192;
    let i3 = (o2 ^ 1) - 1;
    for (let c2 = 0; c2 < 10; c2++)
      r2[c2] &= i3;
    ...
    for (let c2 = 1; c2 < 8; c2++)
      s2 = (e2[c2] + n2[c2] | 0) + (s2 >>> 16) | 0, e2[c2] = s2 & 65535;
    jt(r2);
  }
  update(e2) {
    Or(this);
...
var qs = class extends ke {
  ...
  digestInto(e2) {
    ...
    for (let f6 = 0; f6 < u2; f6++)
      c2.setUint32(4 * f6, l3[f6], i3);
  }
  digest() {
    const { buffer: e2, outputLen: n2 } = this;
    this.digestInto(e2);
...
function Ct(t2, e2) {
  if (typeof e2 != "boolean")
    throw new Error(t2 + " boolean expected, got " + e2);
}
...
function et(t2, e2, n2) {
  ...
  if (typeof n2 == "number" && o2 !== n2)
    throw new Error(t2 + " of length " + n2 + " expected, got " + o2);
  return r2;
}
function ne(...t2) {
  let e2 = 0;
...
function ft(t2, e2, n2, r2) {
  if (!Ee(e2, n2, r2))
    throw new Error("expected valid " + t2 + ": " + n2 + " <= n < " + r2 + ", got " + e2);
}
function Pr(t2) {
  let e2;
...
function Dt(t2, e2, n2 = {}) {
  const r2 = (o2, i3, s2) => {
    ...
    const a = t2[o2];
    if (!(s2 && a === void 0) && !c2(a, t2))
      throw new Error("param " + String(o2) + " is invalid. Expected " + i3 + ", got " + a);
  };
  for (const [o2, i3] of Object.entries(e2))
    r2(o2, i3, false);
...
function Hr(t2, e2, n2) {
  ...
  let r2 = H;
  for (; e2 > q; )
    e2 & H && (r2 = r2 * t2 % n2), t2 = t2 * t2 % n2, e2 >>= H;
  return r2;
}
...
function rc(t2) {
  ...
  for (n2 = t2 - H, r2 = 0; n2 % At === q; n2 /= At, r2++)
    ;
  for (o2 = At; o2 < t2 && Hr(o2, e2, t2) !== t2 - H; o2++)
    if (o2 > 1e3)
  ...
  return function(c2, a) {
    if (c2.pow(a, e2) === c2.neg(c2.ONE))
      throw new Error("Cannot find square root");
    let u2 = r2, l3 = c2.pow(c2.mul(c2.ONE, o2), n2), f6 = c2.pow(a, i3), h4 = c2.pow(a, n2);
    for (; !c2.eql(h4, c2.ONE); ) {
      if (c2.eql(h4, c2.ZERO))
        return c2.ZERO;
      let y4 = 1;
      for (let p = c2.sqr(h4); y4 < u2 && !c2.eql(p, c2.ONE); y4++)
        p = c2.sqr(p);
...
function cc(t2, e2, n2) {
  ...
  let r2 = t2.ONE, o2 = e2;
  for (; n2 > q; )
    n2 & H && (r2 = t2.mul(r2, o2)), o2 = t2.sqr(o2), n2 >>= H;
  return r2;
}
function ac(t2, e2) {
...
function Kr2(t2, e2, n2 = false, r2 = {}) {
  ...
  const c2 = Object.freeze({ ORDER: t2, isLE: n2, BITS: o2, BYTES: i3, MASK: Je(o2), ZERO: q, ONE: H, create: (a) => X(a, t2), isValid: (a) => {
    ...
    if (a.length !== i3)
      throw new Error("Field.fromBytes: expected " + i3 + " bytes, got " + a.length);
    return n2 ? ee(a) : Ot(a);
  } });
...
function Yr(t2, e2) {
  if (!Number.isSafeInteger(t2) || t2 <= 0 || t2 > e2)
    throw new Error("invalid window size, expected [1.." + e2 + "], got W=" + t2);
}
function on(t2, e2) {
  Yr(t2, e2);
  ...
  return { constTimeNegate: rn, hasPrecomputes(n2) {
    ...
    for (let f6 = 0; f6 < s2; f6++) {
      ...
      if (o2 === Zr)
        break;
      let y4 = Number(o2 & a);
      if (o2 >>= l3, y4 > c2 && (y4 -= u2, o2 += ve), y4 === 0)
        continue;
...
function hc(t2, e2, n2, r2) {
  if (fc(n2, t2), lc(r2, e2), n2.length !== r2.length)
  ...
  for (let f6 = u2; f6 >= 0; f6 -= s2) {
    ...
    let h4 = o2;
    for (let y4 = a.length - 1, E2 = o2; y4 > 0; y4--)
      E2 = E2.add(a[y4]), h4 = h4.add(E2);
    if (l3 = l3.add(h4), f6 !== 0)
      for (let y4 = 0; y4 < s2; y4++)
        l3 = l3.double();
  }
  return l3;
}
...
function Ac(t2) {
  ...
  if (!n2.eql(n2.sqr(e2.Gy), s2(e2.Gx)))
  ...
  function a(p) {
    ...
    if (d2 && typeof p != "bigint") {
      if (St(p) && (p = Pt(p)), typeof p != "string" || !d2.includes(p.length))
  ...
  const l3 = tn((p, d2) => {
    ...
    if (p.is0()) {
      if (e2.allowInfinityPoint && !n2.is0(p.py))
        return;
      throw new Error("bad point: ZERO");
    }
    const { x: d2, y: v } = p.toAffine();
    if (!n2.isValid(d2) || !n2.isValid(v))
  ...
  class h4 {
    constructor(d2, v, m2) {
      if (this.px = d2, this.py = v, this.pz = m2, d2 == null || !n2.isValid(d2))
        throw new Error("x required");
      if (v == null || !n2.isValid(v))
    ...
    static fromAffine(d2) {
      const { x: v, y: m2 } = d2 || {};
      if (!d2 || !n2.isValid(v) || !n2.isValid(m2))
    ...
    multiplyUnsafe(d2) {
      ...
      if (this.is0() || d2 === K)
        return this;
      if (!v || E2.hasPrecomputes(this))
        return E2.wNAFCachedUnsafe(this, d2, h4.normalizeZ);
      let { k1neg: N, k1: $, k2neg: B3, k2: A2 } = v.splitScalar(d2), T = O2, S2 = O2, L = this;
      for (; $ > dt || A2 > dt; )
        $ & K && (T = T.add(L)), A2 & K && (S2 = S2.add(L)), L = L.double(), $ >>= K, A2 >>= K;
...
function Ic(t2) {
  ...
  class d2 {
    ...
    recoverPublicKey(w) {
      ...
      if (R3 == null || ![0, 1, 2, 3].includes(R3))
...
function rr2(r2, e2) {
  ...
  function d2(u2) {
    ...
    if (u2.length === 0)
      return "";
    for (var b = 0, x = 0, I2 = 0, D = u2.length; I2 !== D && u2[I2] === 0; )
      I2++, b++;
    for (var j = (D - I2) * l3 + 1 >>> 0, T = new Uint8Array(j); I2 !== D; ) {
      for (var q2 = u2[I2], J2 = 0, K2 = j - 1; (q2 !== 0 || J2 < x) && K2 !== -1; K2--, J2++)
        q2 += 256 * T[K2] >>> 0, T[K2] = q2 % a >>> 0, q2 = q2 / a >>> 0;
      if (q2 !== 0)
        throw new Error("Non-zero carry");
      x = J2, I2++;
    }
    for (var H2 = j - x; H2 !== j && T[H2] === 0; )
      H2++;
    for (var me2 = c2.repeat(b); H2 < j; ++H2)
      me2 += r2.charAt(T[H2]);
    return me2;
  }
  function g(u2) {
    ...
    var b = 0;
    if (u2[b] !== " ") {
      for (var x = 0, I2 = 0; u2[b] === c2; )
        x++, b++;
      for (var D = (u2.length - b) * h4 + 1 >>> 0, j = new Uint8Array(D); u2[b]; ) {
        var T = t2[u2.charCodeAt(b)];
        if (T === 255)
          return;
        for (var q2 = 0, J2 = D - 1; (T !== 0 || q2 < I2) && J2 !== -1; J2--, q2++)
          T += a * j[J2] >>> 0, j[J2] = T % 256 >>> 0, T = T / 256 >>> 0;
        if (T !== 0)
          throw new Error("Non-zero carry");
        I2 = q2, b++;
      }
      if (u2[b] !== " ") {
        for (var K2 = D - I2; K2 !== D && j[K2] === 0; )
          K2++;
        for (var H2 = new Uint8Array(x + (D - K2)), me2 = x; K2 !== D; )
          H2[me2++] = j[K2++];
        return H2;
      }
    }
...
var pr = (r2, e2, t2) => {
  ...
  for (let c2 = 0; c2 < r2.length; ++c2)
    for (a = a << 8 | r2[c2], o2 += 8; o2 > t2; )
      o2 -= t2, n2 += e2[s2 & a >> o2];
  if (o2 && (n2 += e2[s2 & a << t2 - o2]), i3)
    for (; n2.length * t2 & 7; )
      n2 += "=";
  return n2;
};
...
function ai(r2, e2, t2) {
  e2 = e2 || [], t2 = t2 || 0;
  for (var i3 = t2; r2 >= sn3; )
    e2[t2++] = r2 & 255 | oi, r2 /= 128;
  for (; r2 & tn2; )
    e2[t2++] = r2 & 255 | oi, r2 >>>= 7;
...
function Me(r2, i3) {
  ...
  do {
    if (n2 >= a)
      throw Me.bytes = 0, new RangeError("Could not decode varint");
...
*/
//# sourceMappingURL=chunk-H6RBPYSA.js.map
