...
contract P2PLending is P2PReentrancyGuard, P2POwnable {
    ...
    mapping(address => uint256) public userMinCollateralRatio;
    ...
    function setCreditScore(address user, uint256 creditScore) external onlyOwner {
        ...
        userMinCollateralRatio[user] = calculateMinCollateralRatio(creditScore);
    }

    /**
     * @dev Calculate minimum collateral ratio based on credit score
     * Fixed at 85% for all credit scores (undercollateralized)
     */

function calculateMinCollateralRatio(uint256 /*creditScore*/) internal pure returns (uint256) {
    // Fixed 85% collateral requirement for all users
    return 8500; // 85% collateral (8.5 ETH for 10 ETH loan)
    }
    ...
    function createLoanRequest(
        ...
        // Calculate required collateral: 85% of loan amount
        uint256 requiredCollateral = (_loanAmount * 8500) / 10000;
    ...
    function createLenderOffer(
        ...
        require(_minCollateralRatio >= 8500 && _minCollateralRatio <= 9500, "Invalid min collateral ratio");
    ...
    function fundLoan(uint256 _loanId, uint256 _offerId) external payable nonReentrant {
        ...
        // Calculate actual collateral ratio and check against offer minimum
        uint256 actualCollateralRatio = (loan.collateralAmount * 10000) / loan.loanAmount;
        require(actualCollateralRatio >= offer.minCollateralRatio, "Collateral ratio too low for offer");
     ...
     */
    function calculateRequiredCollateral(uint256 loanAmount) external pure returns (uint256) {
        return (loanAmount * 8500) / 10000;
    }
    ...
    function getUserMinCollateralRatio(address user) external view returns (uint256) {
        return userMinCollateralRatio[user];
    }
