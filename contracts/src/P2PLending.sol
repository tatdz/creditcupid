...
const BorrowTab: React.FC<BorrowTabProps> = ({
  ...
  // FIXED: Calculate collateral as 60% of loan amount (what you want)
  const calculateRequiredCollateral = (loan: string): string => {
    const loanValue = parseFloat(loan);
    if (isNaN(loanValue) || loanValue <= 0) return '';
    // For 60% LTV: Collateral = Loan Amount × (LTV/100)
    // But since you want collateral to be 60% of loan, we do:
    const requiredCollateral = loanValue * 0.6; // 60% of loan amount
  ...
  const isValid = parseFloat(loanAmount) > 0 && parseFloat(requiredCollateral) >= minCollateral && parseFloat(loanDuration) >= 30;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            Based on 60% collateral requirement (min collateral: {minCollateral} ETH)
          </p>
          ...
          <div className="p-3 bg-blue-50 rounded-lg">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span>
                ...
                <strong>{requiredCollateral} ETH</strong> as collateral for{' '}
                <strong>{loanDuration || '90'} days</strong>
              </span>
            </div>
            <p className="text-xs text-blue-600 mt-1">
              Note: This represents 60% collateral requirement (collateral is 60% of loan amount)
            </p>
          </div>
        )}
...
export const P2PLending: React.FC<P2PLendingProps> = ({ userCreditScore, userAddress }) => {
  ...
  // Handle loan creation - FIXED: Include duration
