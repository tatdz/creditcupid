...
async function testServerFixed() {
    ...
    try {
        ...
        const health = await (0, supertest_1.default)(baseURL).get('/health').timeout(5000);
        ...
        const credit = await (0, supertest_1.default)(baseURL).get(`/api/credit-data/${testAddress}`).timeout(15000);
        ...
        const protocol = await (0, supertest_1.default)(baseURL).get(`/api/protocol-data/${testAddress}`).timeout(15000);
        ...
        const recommendations = await (0, supertest_1.default)(baseURL).get(`/api/recommendations/${testAddress}`).timeout(15000);
        ...
        const simTypes = await (0, supertest_1.default)(baseURL).get('/api/sandbox/simulation-types').timeout(5000);
        console.log(`   ✅ Status: ${simTypes.status}`);
        if (simTypes.status === 200) {
            console.log(`   🎮 Available Simulations: ${simTypes.body.length}`);
            simTypes.body.forEach((type) => {
        ...
        for (const simType of simulations) {
            ...
            const sim = await (0, supertest_1.default)(baseURL)
        ...
        const enhanced = await (0, supertest_1.default)(baseURL).get(`/api/enhanced-protocol-data/${testAddress}`).timeout(15000);
        ...
        for (const test of errorTests) {
            const response = await (0, supertest_1.default)(baseURL).get(test.path).timeout(5000);
